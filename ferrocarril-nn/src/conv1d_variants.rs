...
impl TextEncoderConv1d {
    ...
    pub fn set_weight_norm(&mut self, weight_g: &Tensor<f32>, weight_v: &Tensor<f32>) -> Result<(), FerroError> {
        // Handle PyTorch weight normalization parameter shapes
        // weight_g can be [out_channels] or [out_channels, 1, 1] depending on PyTorch version
        let expected_weight_g_shape = if weight_g.shape() == &[self.out_channels] {
            vec![self.out_channels]
        } else if weight_g.shape() == &[self.out_channels, 1, 1] {
            // PyTorch sometimes stores weight_g as [C, 1, 1] - reshape to [C]
            let mut reshaped_data = Vec::new();
            for c in 0..self.out_channels {
                reshaped_data.push(weight_g[&[c, 0, 0]]);
            }
            self.weight_g = Parameter::new(Tensor::from_data(reshaped_data, vec![self.out_channels]));
            ...
            return Err(FerroError::new(format!(
                "weight_g shape mismatch: expected [{}] or [{}, 1, 1], got {:?}", 
                self.out_channels, self.out_channels, weight_g.shape()
            )));
        };
        
        assert_eq!(weight_g.shape(), &expected_weight_g_shape,
            "weight_g shape mismatch: expected {:?}, got {:?}", expected_weight_g_shape, weight_g.shape());
        assert_eq!(weight_v.shape(), &[self.out_channels, self.in_channels / self.groups, self.kernel_size],
            "weight_v shape mismatch: expected [{}, {}, {}], got {:?}", 
            self.out_channels, self.in_channels / self.groups, self.kernel_size, weight_v.shape());
...
impl LoadWeightsBinary for PredictorConv1d {
    fn load_weights_binary(
        ...
        let bias = loader.load_component_parameter(component, &format!("{}.bias", prefix))?;
        
        self.weight_g = Parameter::new(Tensor::from_data(vec![1.0; weight.shape()[0]], vec![weight.shape()[0]]));
        self.weight_v = Parameter::new(weight);
