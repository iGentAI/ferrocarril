...
impl LoadWeightsBinary for ProjectionLinear {
    fn load_weights_binary(
        ...
        println!("🔍 ProjectionLinear loading weights for {}.{} (type: {:?})", 
                component, prefix, self.projection_type);
        
        // Try weight normalization format first
        let weight_loaded = if let (Ok(weight_g), Ok(weight_v)) = (
            ...
            println!("Loading ProjectionLinear with weight normalization for {}.{}", component, prefix);
            
            // Handle weight_g shape variations for Linear layers
            let g_data = match weight_g.shape().len() {
                1 => weight_g.data().to_vec(),
                2 if weight_g.shape()[1] == 1 => {
                    // Linear weight_g format [out_features, 1]
                    (0..weight_g.shape()[0])
                        .map(|i| weight_g[&[i, 0]])
                        .collect()
                }
                _ => {
                    return Err(FerroError::new(format!(
                        "Unsupported weight_g shape for Linear: {:?}",
                        weight_g.shape()
                    )));
                }
            };
            
            // Validate weight_v shape
            if weight_v.shape().len() != 2 {
                return Err(FerroError::new(format!(
                    "weight_v must be 2D for Linear, got shape {:?}",
                    weight_v.shape()
                )));
            }
            
            let (out_features, in_features) = (weight_v.shape()[0], weight_v.shape()[1]);
            
            // Reconstruct weight: weight = g * v / ||v||
            let mut normalized_weight = vec![0.0; out_features * in_features];
            
            for o in 0..out_features {
                // Compute L2 norm of weight_v for this output
                let mut norm_sq = 0.0;
                for i in 0..in_features {
                    let val = weight_v[&[o, i]];
                    norm_sq += val * val;
                }
                ...
                // Normalize and scale
                for i in 0..in_features {
                    let idx = o * in_features + i;
                    normalized_weight[idx] = g_val * weight_v[&[o, i]] / norm;
                }
            }
            
            let weight = Tensor::from_data(normalized_weight, vec![out_features, in_features]);
            
            // Update layer configuration if needed
            if out_features != self.out_features || in_features != self.in_features {
                println!("INFO: Adapting ProjectionLinear {}.{} from [{}, {}] to weight shape [{}, {}]",
                        component, prefix, self.out_features, self.in_features, 
        ...
        if !weight_loaded {
            // Try regular weight format - THIS IS COMMON FOR PROJECTION LAYERS
            ...
            match loader.load_component_parameter(component, &format!("{}.weight", prefix)) {
                Ok(weight) => {
                    ...
                    // Validate and potentially adapt dimensions
                    if weight.shape().len() != 2 {
                        return Err(FerroError::new(format!(
                            "ProjectionLinear weight must be 2D, got shape {:?}",
                            weight.shape()
                        )));
                    }
                    
                    let (actual_out, actual_in) = (weight.shape()[0], weight.shape()[1]);
                    
                    if actual_out != self.out_features || actual_in != self.in_features {
                        println!("INFO: Adapting ProjectionLinear {}.{} from [{}, {}] to weight shape [{}, {}]",
                                component, prefix, self.out_features, self.in_features, 
                                actual_out, actual_in);
                        self.out_features = actual_out;
                        self.in_features = actual_in;
                    }
                ...
                Err(e) => {
                    return Err(FerroError::new(format!(
                        "Cannot load ProjectionLinear weight for {}.{} in any format: {}",
                        component, prefix, e
                    )));
                }
            }
        ...
        if let Ok(bias) = loader.load_component_parameter(component, &format!("{}.bias", prefix)) {
            // Adapt bias configuration if needed
            if bias.shape()[0] != self.out_features {
                println!("INFO: Adapting bias size for {}.{} from {} to {}", 
        ...
        println!("✅ ProjectionLinear {}.{} loaded successfully with final shape [{}, {}]",
                component, prefix, self.out_features, self.in_features);
