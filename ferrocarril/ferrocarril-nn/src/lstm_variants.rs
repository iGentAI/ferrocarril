...
impl LSTMGates {
    ...
    fn load_from_stacked_weights(
        ...
        // Extract biases
        self.input_bias = Parameter::new(extract_gate_biases(bias_ih, LSTMGate::Input, hidden_size));
        self.forget_bias = Parameter::new(extract_gate_biases(bias_ih, LSTMGate::Forget, hidden_size));
        self.cell_bias = Parameter::new(extract_gate_biases(bias_ih, LSTMGate::Cell, hidden_size));
        self.output_bias = Parameter::new(extract_gate_biases(bias_ih, LSTMGate::Output, hidden_size));
        
        // Add hidden-hidden biases to input-hidden biases (PyTorch convention)
        ...
        // Combine biases
        for h in 0..hidden_size {
            self.input_bias.data_mut()[&[h]] += hh_input_bias[&[h]];
            self.forget_bias.data_mut()[&[h]] += hh_forget_bias[&[h]];
            self.cell_bias.data_mut()[&[h]] += hh_cell_bias[&[h]];
            self.output_bias.data_mut()[&[h]] += hh_output_bias[&[h]];
        }
...
impl TextEncoderLSTM {
    ...
    pub fn forward_batch_first_with_lengths(
        ...
        let final_hidden = Tensor::from_data(forward_hidden, vec![self.hidden_size]);
        let final_cell = Tensor::from_data(forward_cell, vec![self.hidden_size]);
