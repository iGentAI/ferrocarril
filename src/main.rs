...
enum Commands {
    ...
    Infer {
        ...
        /// Path to a converted Kokoro weights directory. Produce one
        /// with `python3 weight_converter.py --huggingface
        /// hexgrad/Kokoro-82M --output ferrocarril_weights`.
        #[arg(short, long)]
        model: String,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cli.command {
        Some(Commands::Infer { text, phonemes, output, model, voice, speed }) => {
            let config_path = format!("{}/config.json", model);
            let config = Config::from_json(&config_path)?;

            // Load model from the converted binary weights directory.
            let model = FerroModel::load_binary(&model, config)?;
