...
impl FerroModel {
    ...
    pub fn load_voice(&self, voice_name: &str) -> Result<Tensor<f32>, Box<dyn Error>> {
        #[cfg(feature = "weights")]
        {
            ...
            let weights_dir = ["ferrocarril_weights", "../ferrocarril_weights", "../../ferrocarril_weights"]
                .iter()
                .map(std::path::PathBuf::from)
                .find(|p| p.exists());

            let voices_dir = match weights_dir {
                Some(d) => d.join("voices"),
                None => {
                    eprintln!(
                        "ferrocarril: warning: ferrocarril_weights directory not found in cwd or parents"
                    );
                    std::path::PathBuf::new()
                }
            };

            if !voices_dir.as_os_str().is_empty() && !voices_dir.exists() {
                eprintln!(
                    "ferrocarril: warning: voices directory not found at {:?}",
                    voices_dir
                );
            } else if !voices_dir.as_os_str().is_empty() {
